...
public class CameraFlightFollow : MonoBehaviour {
	...
	//What the camera looks at. Generally the targeter.
	public PlayerFlightControl control; 
	//The PlayerFlightControl script that is in play.
	
	public float follow_distance = 3.0f; 
	//How far behind the camera will follow the targeter.
	public float camera_elevation = 3.0f; 
	//How high the camera will rise above the targeter's Z axis.
	public float follow_tightness = 5.0f; 
	//How closely the camera will follow the target. 
	//Higher values are snappier, lower results in a more lazy follow.

	public float rotation_tightness = 10.0f; 
	//How closely the camera will react to rotations, similar to above.
	public float afterburner_Shake_Amount = 2f; 
	//How much the camera will shake when afterburners are active.
	public float yawMultiplier = 0.005f; 
	//Curbs the extremes of input. This should be a really small number. Might need to be tweaked, but do it as a last resort.
	
	public bool shake_on_afterburn = true; 
	//The camera will shake when afterburners are active.
	
	public static CameraFlightFollow instance; 
